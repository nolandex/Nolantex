...
interface ProductGroup {
  groupName?: string // For grouped products like Instagram Booster
  category: string
  subcategory?: string
  items: Product[]
}
...
export default function SecondPage() {
  ...
  // Filter product groups by active category and subcategory
  const filteredProductGroups = productGroups.filter((group) => {
    if (group.category !== activeCategory) return false
    if (activeCategory === "website") {
      return group.subcategory === activeSubcategory
    }
    return true
  })
          ...
          {filteredProductGroups.map((group, groupIndex) => (
            ...
            >
              <div className="p-3">
                ...
                {group.groupName && (
                  <h3
                    className={`text-sm font-bold leading-tight mb-2 ${theme === "dark" ? "text-white" : "text-gray-900"}`}
                  >
                    {group.groupName}
                  </h3>
                )}
                ...
                {group.items.map((product, index) => (
                  <div key={index} className={group.groupName && index !== 0 ? "mt-4 pt-4 border-t border-gray-200 dark:border-gray-700" : ""}>
                    {/* Header with Product Name and Price */}
                    <div className="flex justify-between items-start mb-2">
                      <h4
                        className={`text-xs font-semibold ${theme === "dark" ? "text-gray-200" : "text-gray-800"}`}
                      >
                        {product.name}
                      </h4>
                      <span
